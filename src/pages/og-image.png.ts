...
// Disable prerendering to allow dynamic query parameters
export const prerender = false;

export const GET: APIRoute = async ({ url }) => {

  const title = url.searchParams.get('title') || "SanPhan's Blog";
  const description = url.searchParams.get('description') || 'Thoughts on web development, frontend engineering, and life.';  // Create the HTML template for the OG image - Satori compatible
  const template = html`
